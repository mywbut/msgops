                ...
                <div className="mx-auto max-w-6xl">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                        ...
                        <div className="space-y-2">
                            {[
                                { label: 'General Info', icon: <Settings className="w-4 h-4" />, active: true },
                                { label: 'Team Members', icon: <Users className="w-4 h-4" /> },
                                { label: 'WABA & API', icon: <Code className="w-4 h-4" /> },
                                { label: 'Billing & Plans', icon: <CreditCard className="w-4 h-4" /> },
                                { label: 'Security', icon: <Shield className="w-4 h-4" /> },
                            ].map((item, i) => (
                                <button key={i} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${item.active ? 'bg-white text-[#4F46E5] shadow-sm border border-gray-100' : 'text-gray-400 hover:text-gray-600'}`}>
                                    {item.icon}
                                    {item.label}
                                </button>
                            ))}
